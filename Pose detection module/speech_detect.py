...
def classifyPose(landmarks):
    ...
    if (left_hip_angle >= 160 and left_hip_angle <= 180) and (right_hip_angle>= 180 and right_hip_angle <= 200):
        if (left_knee_angle >= 170 and left_knee_angle <= 190) and (right_knee_angle >=170 and right_knee_angle <= 190):

            if abs(midpoint(points[6], points[8])[0] - mp[0] ) < 30:
                return pose, True, "Good, delivering a speech."

            else:
                return pose, False, "Stand straight!"
        
        else:
            return pose, False, "Your knees :-P"
